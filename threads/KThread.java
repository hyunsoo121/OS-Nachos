...
public class KThread {
	...
	public static void finish() {
		...
		if(currentThread.waitingThread != null) {//join을 호출해서 블락당한 스레드가 있다면 ready상태로 만들고 waitingThread를 null로 초기화
			currentThread.waitingThread.ready();
	...
	public void join() {
		...
		Machine.interrupt().disable(); //인터럽트 방지
		Lib.assertTrue(this != currentThread); //자기자신을 join 메소드로 호출하는 것을 막음
		Lib.assertTrue(this.waitingThread == null); //임의의 쓰레드에 대해 join 메소드를 2번 이상 호출하는 것을 막음

		if(this.status != statusFinished) { //호출한 스레드가 아직 안끝났다면
			this.waitingThread = currentThread;
			currentThread.sleep(); //현재 실행되는 스레드를 waitingThread에 넣고 sleep 시킴
		}

		Machine.interrupt().enable(); //인터럽트 방지해제
